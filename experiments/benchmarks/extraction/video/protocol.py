...
class VideoProtocol:
    ...
    def hybrid_threshold(self, profile: str) -> float:
        ...
        if self.selected_scene_threshold is None or not self.selected_scene_source_run_id:
            raise ValueError(
        ...
        if self.selected_scene_threshold not in self.scene_thresholds:
            raise ValueError("Selected video scene threshold is outside the declared range")
...
def protocol_from_mapping(
    ...
    root = strict_object(
        value,
        "video protocol root",
        {"schema_version", "protocol_version", "seed", "keyframes", "asr", "visual_text", "datasets", "statistics", "selection", "profiles"},
    )
    keyframes = strict_object(
        root["keyframes"], "keyframes",
        {"fixed_intervals_seconds", "scene_thresholds", "hybrid_maximum_gaps_seconds", "include_frame_zero", "frame_sync", "smoke_hybrid_scene_threshold", "selected_scene_threshold", "selected_scene_source_run_id"},
    )
    fixed = increasing_integers(keyframes["fixed_intervals_seconds"], "keyframes.fixed_intervals_seconds")
    gaps = increasing_integers(keyframes["hybrid_maximum_gaps_seconds"], "keyframes.hybrid_maximum_gaps_seconds")
    scenes = tuple(
        number(item, f"keyframes.scene_thresholds[{index}]", minimum=0, maximum=1)
        for index, item in enumerate(sequence(keyframes["scene_thresholds"], "keyframes.scene_thresholds"))
    )
    if not scenes or tuple(sorted(set(scenes))) != scenes:
        raise ValueError("Scene thresholds must be non-empty, unique, and increasing")
    include_zero = boolean(keyframes["include_frame_zero"], "keyframes.include_frame_zero")
    if not include_zero:
        raise ValueError("Video protocol must include frame zero")
    frame_sync = choice(keyframes["frame_sync"], "keyframes.frame_sync", {"vfr"})
    smoke_threshold = number(keyframes["smoke_hybrid_scene_threshold"], "keyframes.smoke_hybrid_scene_threshold", minimum=0, maximum=1)
    ...
    selected = None if selected_raw is None else number(selected_raw, "keyframes.selected_scene_threshold", minimum=0, maximum=1)
    source_raw = keyframes["selected_scene_source_run_id"]
    source_run = None if source_raw is None else string(source_raw, "keyframes.selected_scene_source_run_id")
    if (selected is None) != (source_run is None):
        raise ValueError("Selected scene threshold and source run ID must be set together")
    if selected is not None and selected not in scenes:
        raise ValueError("Selected scene threshold must be in scene_thresholds")
    asr = strict_object(
        root["asr"], "asr",
        {"window_length_seconds", "overlap_seconds", "stitching_method", "stitching_normalization", "maximum_overlap_units"},
    )
    window = number(asr["window_length_seconds"], "asr.window_length_seconds", minimum=0, maximum=30, minimum_exclusive=True)
    ...
    stitching = {
        "method": choice(asr["stitching_method"], "asr.stitching_method", {"normalized_suffix_prefix"}),
        "normalization": choice(asr["stitching_normalization"], "asr.stitching_normalization", {"asr-minimal-v1"}),
        "maximum_overlap_tokens": integer(asr["maximum_overlap_units"], "asr.maximum_overlap_units", minimum=1),
    }
    ...
    datasets = strict_object(root["datasets"], "datasets", {"video_counts", "manifest_duration_tolerance_seconds"})
    counts = strict_object(datasets["video_counts"], "datasets.video_counts", {"smoke", "development", "validation", "locked"})
    video_counts = {name: integer(item, f"datasets.video_counts.{name}", minimum=1) for name, item in counts.items()}
    duration_tolerance = number(datasets["manifest_duration_tolerance_seconds"], "datasets.manifest_duration_tolerance_seconds", minimum=0)
