...
def protocol_from_mapping(
    ...
    root = strict_object(
        ...
        {"schema_version", "protocol_version", "seed", "tokenizer", "strategies", "index", "quality", "statistics", "selection", "resources", "profiles"},
    )
    ...
    declared = load_candidates(DEFAULT_CANDIDATE_PATH, "development")
    declared_strategies = {candidate.split("|", 1)[0] for candidate in declared}
    if set(strategies) != declared_strategies:
        raise ValueError("Protocol strategies must exactly match candidates.yaml")
    index = strict_object(
        root["index"], "index",
    ...
    return ChunkingEmbeddingProtocol(
        ...
        strategies, batch_size, similarity, normalized, vector_dtype,
