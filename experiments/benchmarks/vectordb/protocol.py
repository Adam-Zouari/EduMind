...
class VectorDatabaseProtocol:
    ...
    def concurrency_levels(
        ...
        return tuple(
            sorted({int(level) for row in rows for level in row["concurrency"]})
        )
...
def load_protocol(path: Path = DEFAULT_PROTOCOL_PATH) -> VectorDatabaseProtocol:
    ...
    workloads = strict_object(
        root["workloads"], "workloads", {"smoke", "development", "validation"}
    )
    ...
    divisors = strict_object(
        synthetic_raw["metadata_divisors"],
        "synthetic.metadata_divisors",
        {"scope_50", "scope_10", "scope_1", "scope_01"},
    )
    synthetic = SyntheticSettings(
        integer(synthetic_raw["centroid_count"], "synthetic.centroid_count", minimum=1),
        integer(
            synthetic_raw["records_per_document"],
            "synthetic.records_per_document",
            minimum=1,
        ),
        number(synthetic_raw["cluster_noise"], "synthetic.cluster_noise", minimum=0),
        number(
            synthetic_raw["near_duplicate_fraction"],
            "synthetic.near_duplicate_fraction",
            minimum=0,
            maximum=1,
        ),
        number(
            synthetic_raw["near_duplicate_noise"],
            "synthetic.near_duplicate_noise",
            minimum=0,
        ),
        number(synthetic_raw["query_noise"], "synthetic.query_noise", minimum=0),
        {
            name: integer(value, f"synthetic.metadata_divisors.{name}", minimum=1)
    ...
    grid_raw = strict_object(
        hnsw["grid"], "hnsw.grid", {"m", "ef_construction", "ef_search"}
    )
    grid = {
        name: increasing_integers(grid_raw[name], f"hnsw.grid.{name}")
        for name in grid_raw
    }
    corpus_limit = integer(
        hnsw["validation_corpus_limit"], "hnsw.validation_corpus_limit", minimum=1
    )
    query_limit = integer(
        hnsw["validation_query_limit"], "hnsw.validation_query_limit", minimum=1
    )
    target = number(
        hnsw["target_recall_at_10"], "hnsw.target_recall_at_10", minimum=0, maximum=1
    )
    choice(
        hnsw["tie_breaking"],
        "hnsw.tie_breaking",
    ...
    mutations = integer(
        evaluation["incremental_mutation_count"],
        "evaluation.incremental_mutation_count",
        minimum=1,
    )
    development_filters = _strings(
        evaluation["development_filters"], "evaluation.development_filters"
    )
    validation_filters = _strings(
        evaluation["validation_filters"], "evaluation.validation_filters"
    )
    shortlist = integer(
        evaluation["shortlist_limit"], "evaluation.shortlist_limit", minimum=1
    )
    ...
    if not set(conformance["compound_filters"]) <= metadata_names:
        raise ValueError(
            "Conformance compound filters must be declared metadata fields"
        )
    ...
    batch_sizes = {
        name: integer(value, f"adapters.upsert_batch_sizes.{name}", minimum=1)
    ...
    timeouts = {
        name: number(
            adapters[name], f"adapters.{name}", minimum=0, minimum_exclusive=True
        )
        for name in adapters
        if name.endswith("_seconds")
    }
    pool_limit = integer(
        adapters["connection_pool_limit"], "adapters.connection_pool_limit", minimum=1
    )
    measurement = strict_object(
        root["measurement"], "measurement", {"monitor_storage", "monitor_server_memory"}
    )
    monitor_storage = boolean(
        measurement["monitor_storage"], "measurement.monitor_storage"
    )
    monitor_memory = boolean(
        measurement["monitor_server_memory"], "measurement.monitor_server_memory"
    )
    ...
    return VectorDatabaseProtocol(
        metadata("vector_database", path, root, profiles=profiles),
        workloads,
        synthetic,
        smoke_hnsw,
        grid,
        ...
        pool_limit,
        timeouts["index_readiness_timeout_seconds"],
        timeouts["index_readiness_poll_seconds"],
        timeouts["docker_monitor_interval_seconds"],
        timeouts["docker_stats_timeout_seconds"],
        timeouts["docker_storage_timeout_seconds"],
        timeouts["docker_helper_timeout_seconds"],
        timeouts["docker_restart_timeout_seconds"],
        monitor_storage,
        monitor_memory,
        confidence,
    )
...
def _workload(value: object, label: str) -> None:
    row = strict_object(
        value, label, {"name", "size", "dimension", "queries", "concurrency"}
    )
...
def _hnsw_row(value: object, label: str) -> dict[str, int]:
    ...
    return {
        name: integer(item, f"{label}.{name}", minimum=1) for name, item in row.items()
    }
