"""Audited benchmark metrics with explicit ranges and directions."""
...
def pareto_front(
    ...
    """Indices not dominated on all supplied metrics; direction is max or min."""
    result = []
    for index, row in enumerate(rows):
        ...
        for other_index, other in enumerate(rows):
            ...
            at_least_as_good = all(
                other[name] >= row[name] if direction == "max" else other[name] <= row[name]
...
def randomized_order(values: Sequence[str], *, seed: int = 42) -> list[str]:
    result = list(values)
