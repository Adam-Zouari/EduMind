...
DEFAULT_PROFILE_BY_KIND = {
    ...
    SourceKind.VIDEO: ExtractionProfile(
        "video-baseline", "video-hybrid", "ffmpeg-system", device="cpu", routing="hybrid"
    ),
}
...
def build_default_registry() -> ExtractorRegistry:
    ...
    registrations = [
        ...
        ExtractorRegistration(
            "pypdf", frozenset({SourceKind.PDF}), lambda: PDFExtractor("pypdf"), "extraction", "pdf"
        ),
        ExtractorRegistration(
            "pdfplumber",
...
class ExtractionPipeline:
    ...
    def extract(
        ...
        *,
        ...
        if isinstance(source, ExtractionRequest):
            ...
            prepared_options = self._prepared_model_options(resolved_profile)
            request = replace(
                request,
                source_kind=kind,
    ...
    def _prepared_model_options(self, profile: ExtractionProfile) -> dict[str, str]:
