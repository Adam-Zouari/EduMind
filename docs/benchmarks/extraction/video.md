...
Which deterministic keyframe policy adds useful visible text to a frozen ASR transcript at acceptable cost? Compare fixed interval, scene change, and scene change plus a maximum-interval fallback. The approved document parser and ASR are supplied through prior summary files; video does not reopen those candidate choices.
...
python experiments/benchmarks/extraction/video/run.py --profile standard --document-summary DOCUMENT_SUMMARY_JSON --audio-summary AUDIO_SUMMARY_JSON
python experiments/benchmarks/extraction/video/run.py --profile full --shortlist SUMMARY_JSON --document-summary DOCUMENT_SUMMARY_JSON --audio-summary AUDIO_SUMMARY_JSON
```
